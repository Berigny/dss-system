...
PRICING = {
    # Google: Gemini 2.5 Flash (Estimated at current Flash rates)
    "google/gemini-2.5-flash": {"input": 0.000000075, "output": 0.0000003},
    
    # xAI: Grok 4.3 (Estimated at Grok-Beta rates)
    "x-ai/grok-4.3": {"input": 0.000002, "output": 0.00001},
    
    # OpenAI: GPT-5.1 (Estimated at GPT-4o rates)
    "openai/gpt-5.1-chat": {"input": 0.000005, "output": 0.000015},
    
    # Anthropic: Haiku 4.5 (Estimated at Haiku 3 rates)
...
class LLMClient:
    ...
    def __init__(self):
        ...
        self.max_tokens: int | None = configured_max_tokens if configured_max_tokens > 0 else None
    ...
    async def generate_response(
        ...
        if response is None:
            return {
                "text": "OpenRouter API error: no response returned",
                "cost": 0,
                "tokens": {},
                "model": selected_model,
            }
