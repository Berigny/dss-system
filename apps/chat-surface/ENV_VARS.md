...
- `CODEX_PRINCIPAL_DID` and `KIMI_PRINCIPAL_DID` are required when `prompt_principal_mode` is set to `codex` or `kimi`; an empty value causes the chat proxy to return `400` with `codex_principal_not_configured` or `kimi_principal_not_configured`.
