...
def build_entity_namespace(ledger_id: str, session_id: str) -> str:
    """Resolve the frontend entity namespace.

    Default behavior matches backend LEDGER_NAMESPACE_SOURCE=ledger_id so
    refresh/history reads the same namespace that writes use.
    Set FRONTEND_ENTITY_MODE=session_hash to restore legacy per-session hash mode.
    """
